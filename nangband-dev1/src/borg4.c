...
/*         borg_equips_artifact(ART_ERIRIL, INVEN_WIELD)) */
    ...
    if (borg_prayer_legal_fail(4, 4, 40) || borg_spell_legal_fail(5, 4,40) ||
        (borg_skill[BI_CDEPTH] == 100 && (borg_prayer_legal(4, 4) || borg_spell_legal(5, 4))))
    ...
    for (i = 0; i < (STORE_INVEN_MAX+(INVEN_TOTAL-INVEN_WIELD)); i++)
    {
        ...
        switch (item->tval)
        {
            ...
            case TV_GLOVES:
                ...
                if (cp_ptr->spell_book == TV_MAGIC_BOOK && borg_skill[BI_MAXSP] > 3)
                {
...
#if 0
    /* race bonuses */
    if (borg_race == RACE_ELF) num_resist_lite = 1000;
    if (borg_race == RACE_GNOME) num_free_act = 1000;
    if (borg_race == RACE_DWARF) num_resist_blind = 1000;
    if (borg_race == RACE_HALF_ORC) num_resist_dark = 1000;
    if (borg_race == RACE_HIGH_ELF) num_resist_lite = 1000;
    if (borg_race == RACE_HIGH_ELF) num_see_inv = 1000;
#endif
...
static s32b borg_power_aux2(void)
{
    ...
    if (cp_ptr->spell_book == TV_MAGIC_BOOK && borg_class == CLASS_MAGE)
    {
    ...
    /* Hack -- most gloves hurt magic for spell-casters */
    if (cp_ptr->spell_book == TV_MAGIC_BOOK && (borg_class == CLASS_RANGER ||
