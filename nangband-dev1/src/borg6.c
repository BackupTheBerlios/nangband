...
bool borg_escape(int b_q)
{
         ...
         ((b_q >= avoidance * (15+risky_boost)/10) && !borg_fighting_unique) )
    {
        ...
        if (borg_spell_fail(1, 5, allow_fail-10) ||
            ...
            borg_spell_fail(5, 2, allow_fail + 9) ||
...
static bool borg_heal(int danger )
{
   ...
   /*  Hack -- rest until healed */
    if ( (!borg_skill[BI_ISBLIND] && !borg_skill[BI_ISPOISONED] && !borg_skill[BI_ISCUT] && !borg_goi &&
         ...
         danger <= borg_fear_region[c_y/11][c_x/11] /* && auto_grid[c_y][c_x] != FEAT_MORE */)
    {
...
bool borg_caution(void)
{
    ...
    if (goal_recalling && goal_recalling <= (borg_game_ratio *2) && !borg_skill[BI_CDEPTH])
    {
        ...
        if  (!borg_goi && borg_spell_fail(7, 4, 15) )
        {
        ...
        if ((!borg_speed && borg_spell_fail(3, 3,15)) ||
            (my_oppose_fire + my_oppose_cold + my_oppose_acid +
             my_oppose_elec + my_oppose_pois < 3 && borg_spell_fail(4,4,15)) ||
            (!borg_shield && !borg_goi && borg_spell_fail(7,1,15)) ||
            (!borg_hero && borg_spell_fail(7,0,15)) ||
            (!borg_berserk && borg_spell_fail(7,2,15)))
        {
        ...
        if ((!borg_bless && (borg_prayer_fail(3,0,15) ||
                            borg_prayer_fail(1,4,15) ||
                            borg_prayer_fail(0,3,15))) ||
    ...
    if (stair_more && !goal_recalling)
    {
        ...
        if (borg_skill[BI_CURSP] > borg_skill[BI_MAXSP] * 6 /10 &&
           ((!borg_speed && borg_spell_fail(3, 3,15)) ||
            (my_oppose_fire + my_oppose_cold + my_oppose_acid +
              my_oppose_elec + my_oppose_pois < 3 && borg_spell_fail(4,4,15)) ||
            (!borg_shield && !borg_goi && borg_spell_fail(7,1,15)) ||
            (!borg_hero && borg_spell_fail(7,0,15)) ||
          ...
          (borg_surround && p != 0))
   {
        ...
        (borg_class == CLASS_WARRIOR && borg_skill[BI_AMISSILES] <=0)))
    {
        if (borg_prayer(0, 3) ||
            ...
            borg_spell_fail(7, 2, 25) ||
            borg_spell_fail(7, 0, 25) ||
            borg_activate_artifact(ART_DAL,INVEN_FEET) )
        {
            return (TRUE);
        }
...
static int borg_defend_aux_mass_genocide(void)
{
        ...
        p2 <= (borg_fighting_unique?(avoidance*2/3): (avoidance/2)) )
    {
            ...
            borg_activate_artifact(ART_EONWE, INVEN_WIELD))
        {
            /* Value */
            /* Note this in a log */
            borg_tell(format("%s, Mass Genocide cast in Danger %d, on dlevel %d ",op_ptr->full_name, p1, borg_skill[BI_CDEPTH]));
                return (p1-p2);
        }
    }
    /* Not worth it */
...
bool borg_recover(void)
{
    ...
    if (borg_skill[BI_ISFIXEXP] && (borg_activate_artifact(ART_LUTHIEN, INVEN_OUTER) ||
            borg_prayer(6, 4)) )
    {
        return (TRUE);
    }
