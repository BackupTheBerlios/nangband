 ...
 */
static int monster_light_radius(monster_race *r_ptr)
{
	int rad = 0;

	/* CARRY_LIGHT monsters */
	if (r_ptr->flags2 & (RF2_CARRY_LIGHT))
	{
...
void update_monster_light(void)
{
	...
	for (i = 1; i < m_max; i++)
	{
		...
		/* Is it too far away? (3 is the max radius for monster light) */
		if (m_ptr->cdis > MAX_SIGHT + 3) continue;

		/* Get light radius */
		rad = monster_light_radius(r_ptr);
	...
	for (prio = PRIO_MAX - 1; prio >= PRIO_CARRY_LIGHT; prio--)
	{
		...
		for (l_ptr = priority_list[prio]; l_ptr; l_ptr = l_ptr->next)
		{
			 ...
			 *
			 * Light / darkness paths of radii 1 -- 3, in accordance with
			 * the field of view algorithm of angband 2.8.2 beta -- 3.0.1
			 * and variants based on them.
			 *
			 *     3 3 3
			 *     |\|/|
			 *   3 2 2 2 3
			...
			for (q = 0; q < queue_n; q++)
			{
				...
				if (info & (CAVE_WALL))
				{
					 ...
					 * so that only the correct sides of walls are illuminated.
					 */
