...
static bool room_build(int by0, int bx0, int typ)
{
	...
	/* Restrict level */
	if (p_ptr->depth < room[typ].level && !adult_evil_mode) return (FALSE);
...
static void cave_gen(void)
{
	...
	for (i = 0; i < DUN_ROOMS; i++)
	{
		...
		if (rand_int(DUN_UNUSUAL) < p_ptr->depth || adult_evil_mode)
		{
			/* Roll for room type */
			k = (adult_evil_mode ? rand_int(100) : rand_int(40));
