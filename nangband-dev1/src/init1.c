...
errr parse_z_info(char *buf, header *head)
{
	...
	/* Process '3' for "Maximum magic_book_info[] subindex" */
	else if (buf[2] == '3')
	{
		int max;
		...
		/* Save the value */
		z_info->magic_book_max = max;
	}

	/* Process 'S' for "Maximum magic_spell_info[] subindex" */
	else if (buf[2] == 'S')
	{
		int max;
...
/*
 * Initialize the "magic_book_info" array, by parsing an ascii "template" file
 */
errr parse_book_info(char *buf, header *head)
{
	int i;
	...
	/* Current entry */
	static magic_book_type *b_ptr = NULL;
	...
	if (buf[0] == 'N')
	{
		...
		*s++ = '\0';
		...
		/* Point at the "info" */
		b_ptr = (magic_book_type*)head->info_ptr + i;
	...
	/* Process 'T' for "tval/sval" (one line only) */
	else if (buf[0] == 'T')
	{
		int tval, sval;
	...
	else if (buf[0] == 'I')
	{
		...
		/* Scan for the values */
		if (1 != sscanf(buf+2, "%d",
			            &no_spells)) return (PARSE_ERROR_GENERIC);

		/* Save the values */
		b_ptr->no_spells = no_spells;
	}
...
/*
 * Initialize the "magic_spell_info" array, by parsing an ascii "template" file
 */
errr parse_spell_info(char *buf, header *head)
{
	int i;
	...
	/* Current entry */
	static magic_spell_type *s_ptr = NULL;
	...
	if (buf[0] == 'N')
	{
		...
		*s++ = '\0';
		...
		/* Point at the "info" */
		s_ptr = (magic_spell_type*)head->info_ptr + i;
	...
	else if (buf[0] == 'I')
	{
		...
		if (3 != sscanf(buf+2, "%d:%d:%s",
			            &book, &pos, name)) return (PARSE_ERROR_GENERIC);

		/* Save the values */
		s_ptr->book = book;
		s_ptr->pos = pos;
		strcpy(s_ptr->constant, name);
	}
