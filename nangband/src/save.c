...
static byte savefile_do_byte(byte *v, bool type)
{
	...
	{
		/* Tell teh user we have problems */
		note("Fatal error while reading savefile: out-of-bounds access.  Aborting.");
...
static void savefile_do_block_header(bool type, int ver)
{
	...
	if (type == GET) note(format("Loading a %s %i.%i.%i savefile.", v_name, v_j, v_m, v_p));
...
static errr savefile_do_block_player(bool type, int ver)
{
	...
	{
		/* Warn */
		note(format("Too many hitpoint entries! (%i player levels)", temp));
	...
	{
		/* Warn the user */
		note(format("Invalid player race - %d! Resetting to 0.", p_ptr->prace));
	...
	{
		/* Warn the user */
		note(format("Invalid player class - %d! Resetting to 0.", p_ptr->pclass));
	...
	{
		/* Warn the user */
		note(format("Invalid player level - %d! Resetting to 1.", p_ptr->lev));
	...
	/* Timed resistances */
	savefile_do_s16b(&p_ptr->oppose_acid, type);
	savefile_do_s16b(&p_ptr->oppose_elec, type);
	savefile_do_s16b(&p_ptr->oppose_fire, type);
	savefile_do_s16b(&p_ptr->oppose_cold, type);
	savefile_do_s16b(&p_ptr->oppose_pois, type);
...
static int savefile_check_reload_dungeon(void)
{
	...
	note("This savefile's dungeon data is unrecoverable in this version.  The dungeon");
	note("will be regenerated.  However, if you are in an important situation, it is ");
	note("recommended that you go back to the previous version and then come back to ");
	note("this version when you have \"finished\" the current dungeon level.");
	note(" ");
	note("Do you wish to abort loading? (y/n)");
	...
	if (ch == 'y')
	{
		note("Okay, aborting loading and quitting Angband.  Press any key to continue.");
	...
	{
		note("Okay, dungeon level will be regenerated.");
...
static errr savefile_do_block_dungeon(bool type, int ver)
{
	...
	if ((p_ptr->depth >= MAX_DEPTH) || (p_ptr->depth < 0))
	{
		...
		/* Warn the user */
		note(format("Invalid player depth - %i!", p_ptr->depth));
	}
...
//	if (type == GET) DUNGEON_WID = xmax;
	...
	if (type == GET)
	{
		...
		{
			note(format("Cannot place player at (%d, %d)!", py, px));
			return (-1);
		}
	}
...
static errr savefile_do_block_monlore(bool type, int ver)
{
	...
	{
		/* Warn the user */
		note("Savefile has too many monsters. Not loading monster lore.");
...
static errr savefile_do_block_objlore(bool type, int ver)
{
	...
	{
		/* Warn the user */
		note("Savefile has too many objects. Not loading object lore.");
	...
	if (!err)
	{
		...
		if (!err)
		{
			...
			else if ((header[pos++] == 1) && (header[pos++] < 2))
			{
				...
				/* Note */
				note("Sorry, this savefile cannot be loaded, due to internal changes.");
